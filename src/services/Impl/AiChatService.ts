...
export class AiChatService implements IChatService {
    ...
    async startChat(user: UserType, individualId?: number, message?: string, questionId?: number): Promise<{response: string, individualId: number}> {
        try {
            ...
            const conversationHistory: { role: 'user' | 'assistant'; content: string }[] = chatMessages.map(chatMessage => {
                return {
    ...
    async getChatHistory(userId: number, characterId: number, page: number, pageSize: number): Promise<ChatHistoryResponseDto> {
        try {
            if (!characterId) {
